...
unsigned int get_graphics_reg(unsigned int index) {
	unsigned int saved_addr_reg = inb(GRAPHICS_REG_ADDR);
	outb(GRAPHICS_REG_ADDR, index);
	unsigned int graphics_reg_value = inb(GRAPHICS_REG_DATA);
	outb(GRAPHICS_REG_ADDR, saved_addr_reg); // restore address register
...
void vga_info() {
	...
	char buffer[2];
	itoab(mem_map_select, buffer);
	println(buffer);
	print("Alphanumeric disable: 0b");
	itoa(alpha_dis, buffer);
	println(buffer);
}
